...
O processo de leitura de dados envolve configurar o FSx ONTAP como um bucket S3 privado. Para saber as instruções detalhadas de configuração, consulte link:ai-mlops-fsxn-s3.html["Parte 1 - Integrando o Amazon FSx for NetApp ONTAP (FSx ONTAP) como um bucket S3 privado no AWS SageMaker"]
