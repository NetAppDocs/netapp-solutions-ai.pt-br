...
summary: Nesta configuração, mostramos como ler e escrever tópicos no armazenamento de objetos do Kafka diretamente usando o conector de coletor S3 do Kafka.  Para este teste, usamos um cluster Confluent autônomo, mas esta configuração é aplicável a um cluster distribuído. 
---
...
. Baixe o Confluent Kafka do site da Confluent.
. Descompacte o pacote em uma pasta no seu servidor.
. Exporte duas variáveis.
+
....
Export CONFLUENT_HOME=/data/confluent/confluent-6.2.0
export PATH=$PATH:/data/confluent/confluent-6.2.0/bin
....
. Para uma configuração autônoma do Confluent Kafka, o cluster cria uma pasta raiz temporária em `/tmp` . Ele também cria Zookeeper, Kafka, um registro de esquema, connect, um ksql-server e pastas do centro de controle e copia seus respectivos arquivos de configuração de `$CONFLUENT_HOME` .  Veja o exemplo a seguir:
+
....
...
....
. Configurar o Zookeeper.  Você não precisa alterar nada se usar os parâmetros padrões.
+
....
...
. Criamos um arquivo myid em `/tmp/confluent.406980/zookeeper/data` com um ID único:
+
....
...
. Instalar o Kafka Connect usando `confluent-hub` .
+
....
...
. Por padrão, `confluentinc-kafka-connect-s3` está instalado em `/data/confluent/confluent-6.2.0/share/confluent-hub-components/confluentinc-kafka-connect-s3` .
...
root@stlrx2540m1-108:~#
....
. Pare os serviços do Confluent e reinicie-os.
+
....
...
root@stlrx2540m1-108:/data/confluent/confluent-6.2.0/bin#
....
. Configure o ID de acesso e a chave secreta no `/root/.aws/credentials` arquivo.
+
....
...
root@stlrx2540m4-01:~#
....
. Configure o arquivo de propriedades s3-sink para configuração do s3 e do bucket.
+
....
...
root@stlrx2540m1-108:~#
....
. Importe alguns registros para o bucket s3.
+
....
...
root@stlrx2540m1-108:~#
....
. Verifique o log para ter certeza de que o s3-sink está pronto para aceitar tópicos.
+
....
root@stlrx2540m1-108:~# confluent local services connect log
....
. Confira os tópicos em Kafka.
+
....
...
root@stlrx2540m1-108:~#
....
. Verifique os objetos no bucket s3.
+
....
...
A Instaclustr oferece suporte a conectores Kafka Connect e seus detalhes. link:https://github.com/instaclustr/kafka-connect-connectors["Mais detalhes"]. A Instaclustr fornece conectores adicionais. link:https://www.instaclustr.com/support/documentation/kafka-connect/bundled-kafka-connect-plugins/["seus detalhes"]
