...
summary: Os dados estão disponíveis no NFS e acessados do S3 pelo AWS SageMaker. 
---
...
[role="lead"]
Os dados estão disponíveis no NFS e acessados do S3 pelo AWS SageMaker.



== Requisitos de tecnologia

Você precisa do NetApp BlueXP, NetApp Cloud Volumes ONTAP e AWS SageMaker Notebooks para o caso de uso de dualidade de dados.
...
* AWS SageMaker
* Aprendizado de máquina validado de Jupyter Notebooks
...
Nesta validação, usamos a AWS.  Também é aplicável ao Azure e ao Google Cloud.  Para criar um conector BlueXP na AWS, conclua as seguintes etapas:

. Usamos as credenciais baseadas no mcarl-marketplace-subscription no BlueXP.
. Escolha a região adequada para seu ambiente (por exemplo, us-east-1 [N. Virginia]) e selecione o método de autenticação (por exemplo, Assumir função ou chaves da AWS).  Nesta validação, usamos chaves da AWS.
. Forneça o nome do conector e crie uma função.
. Forneça os detalhes da rede, como VPC, sub-rede ou par de chaves, dependendo se você precisa de um IP público ou não.
. Forneça os detalhes do grupo de segurança, como acesso HTTP, HTTPS ou SSH do tipo de origem, como qualquer lugar e informações de intervalo de IP.
. Revise e crie o Conector BlueXP .
...
. Efetue login na interface do usuário do conector no portal BlueXP ou use o endereço IP para acesso pelo navegador.
...
Para criar uma instância do Cloud Volumes ONTAP no BlueXP, conclua as seguintes etapas:

. Crie um novo ambiente de trabalho, selecione o provedor de nuvem e selecione o tipo de instância do Cloud Volumes ONTAP (como CVO único, HA ou Amazon FSx ONTAP para ONTAP).
. Forneça detalhes como o nome e as credenciais do cluster Cloud Volumes ONTAP .  Nesta validação, criamos uma instância do Cloud Volumes ONTAP chamada `svm_sagemaker_cvo_sn1` .
. Selecione os serviços necessários para o Cloud Volumes ONTAP.  Nesta validação, optamos por apenas monitorar, então desabilitamos *Data Sense & Compliance* e *Backup to Cloud Services*.
. Na seção *Localização e conectividade*, selecione a região da AWS, a VPC, a sub-rede, o grupo de segurança, o método de autenticação SSH e uma senha ou um par de chaves.
. Selecione o método de carregamento.  Usamos *Professional* para esta validação.
. Você pode escolher um pacote pré-configurado, como *POC e pequenas cargas de trabalho*, *cargas de trabalho de produção de dados de banco de dados e aplicativos*, *DR com boa relação custo-benefício* ou *cargas de trabalho de produção de alto desempenho*.  Nesta validação, escolhemos *Poc e Small Workloads*.
...
. Escolha um tipo de disco de perfil e uma política de camadas.  Nesta validação, desabilitamos *Eficiência de armazenamento* e *SSD de uso geral – Desempenho dinâmico*.
. Por fim, revise e crie a instância do Cloud Volumes ONTAP .  Aguarde de 15 a 20 minutos para que o BlueXP crie o ambiente de trabalho do Cloud Volumes ONTAP .
. Configure os seguintes parâmetros para habilitar o protocolo Duality.  O protocolo Duality (NFS/S3) é suportado pelo ONTAP 9.  12.1 e posteriores.
+
.. Nesta validação, criamos uma SVM chamada `svm_sagemaker_cvo_sn1` e volume `vol1` .
.. Verifique se o SVM tem suporte de protocolo para NFS e S3.  Caso contrário, modifique o SVM para suportá-los.
+
....
...
. Crie e instale um certificado CA, se necessário.
...
sagemaker_cvo_sn1::*>
....
. Verifique os detalhes agregados.
+
....
...
Para criar um AWS Notebook a partir do AWS SageMaker, conclua as seguintes etapas:

. Certifique-se de que o usuário que está criando a instância do Notebook tenha uma política do IAM AmazonSageMakerFullAccess ou faça parte de um grupo existente que tenha direitos AmazonSageMakerFullAccess.  Nesta validação, o usuário faz parte de um grupo existente.
. Forneça as seguintes informações:
+
** Nome da instância do notebook.
** Tipo de instância.
** Identificador de plataforma.
** Selecione a função do IAM que tem direitos AmazonSageMakerFullAccess.
** Acesso root – habilitar.
** Chave de criptografia - Selecione nenhuma criptografia personalizada.
** Mantenha as opções padrão restantes.


. Nesta validação, os detalhes da instância do SageMaker são os seguintes:
+
image:cdm-fod-002.png["Captura de tela representando a etapa."]

+
image:cdm-fod-003.png["Captura de tela representando a etapa."]

. Inicie o AWS Notebook.
+
image:cdm-fod-004.png["Captura de tela representando a etapa."]

. Abra o laboratório Jupyter.
+
image:cdm-fod-005.png["Captura de tela representando a etapa."]

. Efetue login no terminal e monte o volume Cloud Volumes ONTAP .
+
....
...
sh-4.2$
....
. Verifique o bucket criado no volume Cloud Volumes ONTAP usando os comandos da AWS CLI.
+
....
...
Nesta validação, usamos um conjunto de dados da DBpedia, um esforço comunitário de crowdsourcing, para extrair conteúdo estruturado das informações criadas em vários projetos da Wikimedia.

. Baixe os dados do local do GitHub da DBpedia e extraia-os.  Utilize o mesmo terminal usado na seção anterior.
+
....
sh-4.2$ wget
--2023-02-14 23:12:11--
...
sh-4.2$
....
. Copie os dados para o local Cloud Volumes ONTAP e verifique-os no bucket do S3 usando a AWS CLI.
+
....
sh-4.2$ df -h
Filesystem          Size  Used Avail Use% Mounted on
...
sh-4.2$
....
. Execute a validação básica para garantir que a funcionalidade de leitura/gravação funcione no bucket S3.
+
....
...
== Validar o aprendizado de máquina dos Jupyter Notebooks

A validação a seguir fornece os modelos de construção, treinamento e implantação de aprendizado de máquina por meio da classificação de texto usando o exemplo SageMaker BlazingText abaixo:
...
....
. Na etapa seguinte, os dados(`dbpedia_csv` ) é baixado do bucket s3 `ontapbucket1` para uma instância do Jupyter Notebook usada em aprendizado de máquina.
+
....
...
....
. Carregue o conjunto de dados formatado e de treinamento no S3 para que ele possa ser usado pelo SageMaker para executar tarefas de treinamento.  Em seguida, carregue dois arquivos no bucket e no local do prefixo usando o Python SDK.
+
....
ln [8]: %%time
...
Wall time: 1.32 s
....
. Configure um local de saída no S3 onde o artefato do modelo é carregado para que os artefatos possam ser a saída do trabalho de treinamento do algoritmo.  Criar um `sageMaker.estimator.Estimator` objeto para iniciar o trabalho de treinamento.
+
....
...
....
. Defina o SageMaker `Estrimator` com configurações de recursos e hiperparâmetros para treinar a classificação de texto no conjunto de dados DBPedia usando o modo supervisionado em uma instância c4.4xlarge.
+
....
...
....
. Prepare um handshake entre os canais de dados e o algoritmo.  Para fazer isso, crie o `sagemaker.session.s3_input` objetos dos canais de dados e mantê-los em um dicionário para o algoritmo consumir.
+
....
...
....
. Após a conclusão do trabalho, uma mensagem de Trabalho Concluído será exibida.  O modelo treinado pode ser encontrado no bucket S3 que foi configurado como `output_path` no estimador.
+
....
...
....
. Por padrão, o modelo retorna uma previsão com a maior probabilidade.  Para recuperar o topo `k` previsões, conjunto `k` no arquivo de configuração.
+
....
...
....
. Exclua o ponto de extremidade antes de fechar o notebook.
+
....
