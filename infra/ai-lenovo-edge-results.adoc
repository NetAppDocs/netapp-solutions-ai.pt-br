...
summary: 'Uma infinidade de testes foi realizada para avaliar o desempenho da arquitetura proposta. Existem seis cargas de trabalho diferentes, como classificação de imagens, detecção de objetos pequenos, detecção de objetos grandes, imagens médicas, conversão de fala em texto e processamento de linguagem natural (NLP), que você pode executar em três cenários diferentes: offline, fluxo único e fluxo múltiplo.' 
---
